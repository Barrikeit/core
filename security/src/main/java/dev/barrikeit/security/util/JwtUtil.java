 ...
 *
 * Uses HMAC-SHA256 (HS256) with a key derived from the configured secret
 * via SHA-256 hashing ({@link HashEncodeUtil#sha256Bytes(String)}).
 *
 * Registered as a Spring {@code @Component} so it can be injected into
 * filters, interceptors, and services in child projects.
 *
 * Token structure:
 *   Standard claims: jti, sub (username), iss (issuer), iat, exp
 *   Custom claims:   user (UUID), roles (List<String>), authorities (List<String>), refreshable (boolean)
 */
public class JwtUtil {
    ...
    private String buildToken(BasicUserDetails user, String jti,
                               long expirationSeconds, boolean refreshable) {
     ...
     * Throws JJWT exceptions on invalid or expired tokens — callers handle them.
     */
